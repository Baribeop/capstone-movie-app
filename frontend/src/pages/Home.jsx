...
function Home() {
    ...
    const fetchMovies = async () => {
      try {
        if (location.state?.searchResults) {
          ...
          const res = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/movies/discover?page=1`);
  ...
  const loadMoreItems = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/movies/discover?page=${nextPage}`);
